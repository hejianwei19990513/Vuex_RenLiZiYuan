<template>
  <div>
    <!-- action为什么给#， 因为前面我们讲过了，我们要上传到腾讯云，需要自定 义的上传方式,action给个#防止报错 -->
...
var cos = new COS({
  SecretId: 'AKIDEfswmLvofCeaIoCB18yZv1tWIn32KCNO', // 身份识别 ID
  SecretKey: 'vE1UgbGdk11pMcbHFyUkkTt4yd1kDPK4' // 身份密钥
})
export default {
  data: function() {
    return {
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      // file要删除的文件, fileList删除该文件后的数组
      this.fileList = this.fileList.filter(item => item.uid !== file.uid)
    },
    // 这个钩子添加文件，上传成功以及上传失败时都会执行，这里会执行添加文件和上传失败调用两次，所以不能push
