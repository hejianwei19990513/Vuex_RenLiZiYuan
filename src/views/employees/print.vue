...
<script>
import { getUserPersonalInfo, getUserJobInfo } from '@/api/employees'
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getUserPersonalInfo() : this.getUserJobInfo()
  },
  // 组件更新
  methods: {
    async getUserPersonalInfo() {
      this.formData = await getUserPersonalInfo(this.userId) // 获取个人基本信息
    },
