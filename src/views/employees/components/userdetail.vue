...
import { getUserPersonalInfo, saveUserDetailById, saveUserPersonalInfo } from '@/api/employees'
export default {
  data: function() {
    return {
  ...
  methods: {
    async getUserDetailInfo() {
      ...
      this.formData = await getUserPersonalInfo(this.userId)
    ...
    async savePersonal() {
      try {
        ...
        await saveUserPersonalInfo({ ...this.formData, staffPhoto: fileList.length ? fileList[0].url : ' ' })
