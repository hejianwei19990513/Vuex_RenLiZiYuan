...
module.exports = {
    ...
    proxy: {
     ...
     '/api': {
       ...
       changeOrigin: true, // 只有这个值为true的情况下 才表示开启跨域
        // 路径重写
      //  pathRewrite: {
      //    // 重新路由  localhost:8888/api/login  => http://ihrm-java.itheima.net/login
  ...
  chainWebpack(config) {
    ...
    // 注入cdn变量（这行代码会在打包的时候执行，就会将cdn变量注入到public文件夹下的index.html模板）
    config.plugin('html').tap(args => {
      // args 是注入public文件夹下的index.html模板的一个变量
      args[0].cdn = cdn
      return args
    })
